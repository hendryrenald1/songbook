...
class App extends React.Component {
  ...
  handleChangeRecording = (event) =>
    this.setState({ recording: event.target.value });
  handleChamgeLyrics = (event) => this.setState({ lyrics: event.target.value });
  handleChangeChords = () =>
    this.setState((prevState) => ({ chorded: !prevState.chorded }));
  ...
  render() {
    ...
    return (
      <div className="songInput">
        <h2>Song Name: {this.state.name}</h2>
        ...
        <p>
          <textarea
            ...
            onChange={this.handleChamgeLyrics}
          />
        </p>
