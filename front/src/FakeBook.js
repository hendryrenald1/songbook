...
class FakeBook extends React.Component {
  ...
  saveSong = (song) => {
    axios
      .post(apiUrl, song, {
        headers: { authorization: "bearer " + this.state.admin },
      })
  ...
  deleteSong = (song) => {
    if (!window.confirm(`Haluatko varmasti poistaa kappaleen "${song.name}"?`))
      return;
    axios
      .delete(apiUrl + "/" + song._id, {
        headers: { authorization: "bearer " + this.state.admin },
      })
      .then(() => this.getSongs())
  ...
  prepareContent = () => {
    ...
    if (edit) {
      return (
        <SongInput song={selected} handleSongInput={this.handleSongInput} />
      );
    }
  ...
  administer = (action) => {
    switch (action) {
      ...
      case "logout":
        const selected =
          this.state.selected === "insertnew" ? "" : this.state.selected;
  ...
  render() {
    ...
    return (
      <div className="supercontainer">
        ...
        {this.state.admin && (
          <AdminPanel edit={this.state.edit} administer={this.administer} />
        )}
        <div className="row">
          <div className="col header">
            <h1 className="siteHeader">Song Book</h1>
          </div>
        </div>
        ...
        <div className="row">
          <div className="col content">
            ...
            <div className="content-main dark">{content}</div>
          </div>
        </div>
      </div>
    );
